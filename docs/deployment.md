...
| HTTP 요청 빈도 | 인증된 사용자 ID별 분당 240회, 초과 시 429(`user_rate_limit`)·Retry-After. 시간에 비례해 회복하므로 창이 끝나기를 기다리지 않는다 |
| 인증 실패 빈도 | 피어 IP별 분당 20회. 소진하면 그 IP의 요청은 인증 검사 전에 429(`auth_rate_limit`). `/mcp` Bearer 실패와 `/internal/` 공유 비밀 실패를 함께 센다. 백엔드 장애·타임아웃은 세지 않는다 |
| 빈도 제한 상태 | 키(IP·사용자 ID) 최대 4096개. 상한에 닿으면 회복 완료·최장 미사용 항목부터 정리하며 새 키를 거부하지 않는다 |
| 프록시 경유 요청 | `Forwarded`, `X-Forwarded-*`, `X-Real-IP`, `Via` 등 프록시 헤더가 있으면 경로와 무관하게 403(`proxy_request_denied`). loopback이 아닌 피어는 403(`peer_denied`) |
...
현재 구현은 이를 **강제한다**. 프록시 헤더가 붙은 요청과 loopback이 아닌 피어의 연결은 fail-closed로 거부하므로, 실수로 `proxy_pass http://127.0.0.1:8081`을 걸어도 외부 요청이 MCP나 `/internal/delegations`에 닿지 않는다. 이 헤더로 실제 클라이언트 IP를 복원하지 않는다 — loopback 전용 서버에는 신뢰할 수 있는 프록시가 없으므로 헤더의 존재 자체가 거부 사유다. 아래 프록시 설정은 위 선행 조건을 구현하고 수신 정책을 함께 바꾼 뒤에야 의미가 있다.
