...
func Register(server *mcp.Server, client *ableops.Client, logger *slog.Logger, options ...Option) []string {
	...
	groupLag := inputSchema(true, true, false)
	groupLag["properties"].(map[string]any)["topic_name"] = map[string]any{"type": "string", "minLength": 1, "maxLength": 249, "description": "선택. 지정하면 이 Topic의 파티션·topicLag만 남깁니다(limit보다 먼저 적용). 생략하면 그룹 전체입니다."}
	register(server, s, "get_consumer_group_lag", "명시한 클러스터와 Consumer Group의 파티션별 Lag·Offset 및 기존 판정을 조회합니다. topic_name으로 한 Topic만 좁힐 수 있습니다. 실패를 Lag 0으로 해석하지 않습니다.", groupLag, func(i GroupLagInput) string { return i.ClusterID }, s.groupLag)
	...
	registerThroughputTools(server, s)
